...
_ICON_SETS: dict[str, tuple[Path, dict[str, tuple[str, ...]]]] = {
    "home": (
        _PROJECT_ROOT / "assets" / "dashboard_icons",
        {
            "my_pins": ("my_pins", "pin"),
    ...
    "settings": (
        _PROJECT_ROOT / "assets" / "settings_icons",
        {
...
def resolve_icon_path(icon_id: str, icon_set: str = "home") -> Path | None:
    ...
    pack = _ICON_SETS.get(icon_set)
    if pack is None:
        _PATH_CACHE[cache_key] = None
        return None
    directory, candidates = pack
    names = candidates.get(icon_id, (icon_id,))
